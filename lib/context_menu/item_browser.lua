...
local function generate_random_string(length)
    local output = ""
    for i=1,length do
        output = output .. string.char(math.random(97, 97 + 25))
    end
    return output
end
...
browser.browse_item = function(parent_menu, this_item, add_item_menu_function, browse_params)
    if browser.state.name == nil then browser.state.name = generate_random_string(10) end
    ...
    if this_item.items ~= nil then
        ...
        local search_command = "search"..browser.state.name..browser.state.search_menu_counter
